...
function checksCreateTodosUserAvailability(request, response, next) {
  ...
  if (user.pro === false && user.todos.length === 10) {
    return response.status(403).json({ error: "Limit todos ultrapassed!" });
  }
...
function checksTodoExists(request, response, next) {
  ...
  const verifyIsUUUID = validate(id);
  const todoExists = users[userIndex].todos.find((todo) => todo.id === id);
  ...
  if (!verifyIsUUUID) {
    return response.status(404).json({ error: "Id is not uuid() valid!" });
  }
